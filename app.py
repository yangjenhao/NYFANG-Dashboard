...
st.markdown(f"""
    ...
    .stButton>button {{ width: 100%; border-radius: 0px !important; border: 1px solid {COLORS['gold']} !important; background-color: transparent !important; color: {COLORS['gold']} !important; font-size: 0.7rem; }}
    </style>
...
def fetch_data(p):
    ...
    # 設定抓取策略
    if p == "1d":
        fetch_p, interval = "2d", "1m" # 抓2天確保有昨日收盤價可算漲跌
    elif p == "5d":
        fetch_p, interval = "10d", "1d" # 5日僅看收盤價
    else:
...
try:
    ...
    for t in returns.index:
        ...
        contrib_df.loc[t] = impact * (pts / total_impact) if abs(total_impact) > 1e-6 else 0
    ...
    if not available_data.empty:
        ...
        with col1:
            ...
            fig_idx.update_layout(
                ...
                xaxis=dict(showgrid=False, color=COLORS['muted'], rangebreaks=rb, type='date'),
        ...
        with col2:
            ...
            fig_bar.update_layout(
                template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                margin=dict(l=0, r=0, t=30, b=0), height=400,
                title=dict(text=f"CONTRIBUTION ({plot_time.strftime('%Y-%m-%d %H:%M')})", font=dict(color=COLORS['gold'], size=14))
            )
