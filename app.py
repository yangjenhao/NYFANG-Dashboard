...
def get_base64_logo(domain):
    """後端抓取 Logo 並轉為 Base64，徹底解決前端擋廣告套件(Adblock)導致圖片破圖的問題"""
    try:
...
try:
    ...
    with col2:
        ...
        # 增加 margin 左側空間 (l=90) 以防 Logo 跑到畫面外
        fig_bar.update_layout(
            template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
            margin=dict(l=90, r=10, t=20, b=10), height=420, 
        ...
        for ticker in row.index:
            ...
            b64_img = get_base64_logo(domain)
            if b64_img:
                fig_bar.add_layout_image(
                    dict(
                        source=b64_img,
                        xref="paper", yref="y",
                        x=-0.05, y=ticker,        # 微調 x 座標，定位在 Y 軸文字左側
                        sizex=0.8, sizey=0.8,     # 控制 Logo 大小
                        sizing="contain",
