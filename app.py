...
try:
    ...
    with c1: st.markdown(f'<div class="metric-card"><p style="color:{COLORS["gold"]}; font-size:0.7rem;">VALUE</p><h2 style="color:{color}">{end[INDEX_SYMBOL]:,.2f}</h2></div>', unsafe_allow_html=True)
    with c2: st.markdown(f'<div class="metric-card"><p style="color:{COLORS["gold"]}; font-size:0.7rem;">SHIFT</p><h2 style="color:{color}">{total_change:+.2f}</h2></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="metric-card"><p style="color:{COLORS["gold"]}; font-size:0.7rem;">VAR %</p><h2 style="color:{color}">{(total_change/start[INDEX_SYMBOL]*100):+.2f}%</h2></div>', unsafe_allow_html=True)

    # Contribution Calculation
    returns = (end[OFFICIAL_TICKERS] / start[OFFICIAL_TICKERS]) - 1
    raw_impacts = returns * 0.1
    row = (raw_impacts * (total_change / raw_impacts.sum())).sort_values(ascending=True)
    ...
    with col1:
        ...
        fig_idx.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=450, margin=dict(t=30, b=10))
    ...
    with col2:
        ...
        for i, ticker in enumerate(row.index):
            ...
            logo_imgs.append(dict(
                ...
                xref="paper", yref="y", x=-0.14, y=i,
                sizex=0.07, sizey=0.7, xanchor="right", yanchor="middle"
            ))
        ...
        fig_bar.update_layout(
            ...
            height=450, margin=dict(l=110, r=50, t=50, b=20),
