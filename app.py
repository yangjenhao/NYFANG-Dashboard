...
try:
    ...
    with col2: # 個股貢獻度
            ...
            for i, ticker in enumerate(row.index):
                ...
                # 針對 MU (Micron) 加入白色圓形襯底，解決深色 Logo 在黑色背景隱身的問題
                if ticker == "MU":
                    logo_imgs.append(dict(
                        source="https://upload.wikimedia.org/wikipedia/commons/a/ad/White_circle.svg", # 向量白色圓形
                        xref="paper", yref="y", x=-0.12, y=ticker,
                        sizex=0.065, sizey=0.65, # 略小於 Logo 邊界
                        xanchor="right", yanchor="middle", sizing="contain",
                        opacity=0.9, layer="below" # 放在 Logo 下方
                    ))
